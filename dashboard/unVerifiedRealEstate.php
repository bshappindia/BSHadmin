...
<body class="">
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="javascript:;">Un verified Real estate</a>
          </div>
  ...
  <script type="text/javascript">
    ...
    $.ajax({
            url : "../mw1.php?s=unVerifiedRealEstate",
            dataType:"JSON",
            type: "GET",
            async:false,
            success:function(data){
                ...
                str += `<div class="col-md-4" id = "${data[index]['id'] + "_rootElement"}">
                        <div class="card" >
                            ...
                            <div class="card-body">
                            <h3 class="card-title" style="display:inline"><b>${data[index].propertyType}</b> for <b>${data[index].transactionType}</b></h3>
                            <h4 ><b>Price :</b>${data[index].price}</h4>
                            
                            <button data-toggle="collapse" data-target="#${data[index].id}">see more</button>
                            <div id="${data[index].id}" class="collapse">
                                <div>
                                    <h3><b>Owner :</b> ${data[index].owner}</h3>
                                    <h4><b>Space</b> : ${data[index].space}</h4>
                                    <h4><b>Price :</b> ${data[index].price}</h4>
                                    <h4><b>Address</b> ${data[index].address}</h4>
                                </div>
                                <h4> <b>Phone : </b> ${data[index].contact}</h4>
                                <hr style="border-top: 2px solid #bbb;">

                                <h4><b>${data[index].details}</b></h4>
                            </div>
                            </div>
                            <div class="card-footer">
                            <button type="button" class="btn btn-primary verifyButton" oid="${data[index].id}" t="realEstate">Verify</button>
                            <button type="button" class="btn btn-secondary deleteButton" oid = "${data[index].id}" t="realEstate">Delete</button>
                            </div>
                        </div>
                        </div>`;
